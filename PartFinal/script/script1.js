...
divViewbar.textContent = `Registered Courses : ${register.studyTable.length}     `

const alink =document.createElement('a');
alink.setAttribute('href',"./home.html");
alink.textContent = "View registration history";
...
header2.textContent ='All Courses ::';
...
function showSubject(subject){
    ...
    buttonRegister.addEventListener('click',()=>{
        ...
        col4.textContent = "Registered";
        document.querySelector('#view-bar').textContent = `Registered Courses : ${register.studyTable.length}     `;
    });
...
function showButton(){
    ...
    registerList.filter((study)=>{
        if(!(study.subjectId.includes(registerList.subjectId))){
            ...
            console.log(MatchingId.lastElementChild.textContent = "Registered");
        }
    });
}
showButton();